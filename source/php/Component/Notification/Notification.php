...
class Notification extends \ComponentLibrary\Component\BaseController
{
    public function init()
    {
        ...
        if (!$animation['onPageLoad'])
            $this->data['classList'][] = 'u-display--none';
        if ($autoHideDuration)
            $this->data['attributeList']['autoHideDuration'] = $autoHideDuration;
        if ($maxAmount)
            $this->data['attributeList']['maxAmount'] = $maxAmount;
