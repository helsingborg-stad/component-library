...
class Hero extends \ComponentLibrary\Component\BaseController
{
    public function init()
    {
        ...
        //Overlay
        if (
            empty($contentBackgroundColor)
            && ($title || $paragraph || $byline || $meta)
            && $this->data['heroView'] !== 'callToActions'
        ) {
    ...
    private function hasContent(): bool
    {
        ...
        if (!$stringEmpty($this->data['meta']))
            return true;
        if (!$stringEmpty($this->data['title']))
            return true;
        if (!$stringEmpty($this->data['byline']))
            return true;
        if (!$stringEmpty($this->data['paragraph']))
            return true;
        if (!empty($this->data['buttonArgs']))
            return true;
